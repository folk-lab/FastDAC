...
void UART::begin(unsigned long baudrate) {
	...
	if (_serial->obj != NULL) {
		...
		_serial->obj->set_blocking(false);
		//_serial->obj->attach(mbed::callback(this, &UART::tx_empty), mbed::SerialBase::TxIrq);
	}
}
...
void UART::on_rx() {
	//This probably breaks using a USB to Serial adapter on the arduino, but we don't care
	//readable() spends a bunch of time locking a USB peripheral that isn't being used
	char c;
...
void UART::tx_empty() {
	...
	{
		_serial->obj->attach(NULL, mbed::SerialBase::TxIrq);
	}		
